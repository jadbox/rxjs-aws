...
(function(root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        root.RxPlus = root.Rx = factory(root, {}, root.RxPlus);
    }
        ...
        rxo.prototype.aws = {
            ...
            'S3': function(params) {
                ...
                wrapper.getObjectAsString = function getObjectAsStringMethod(url) {
                    ...
                    var bucket = url.split('/')[0];
                    url = url.replace(bucket + '/', '');
                    console.log("Getting from s3", "bucket: " + bucket, "key: " + url);
                    var obj = {
                        Bucket: bucket,
                        Key: url
                    };
                    return wrapper.getObject(obj).pluck('Body').mapToString();
                };
            ...
            'SNS': function(params) {
                var wrapper = {};
                this._dynamodb = wrapAPI(aws.SNS, params, wrapper);

                return wrapper;
            },
            'SQS': function(params) {
                var wrapper = {};
                this._dynamodb = wrapAPI(aws.SQS, params, wrapper);
