const rx = global.Rx || require('rxjs/Rx');
...
var rxo = rx.Observable,
  rxn = rx.Observable.bindNodeCallback,
  ...
  return {
    ...
    'S3': function(params) {
      ...
      wrapper.listBucketNames = function listBucketNamesMethod() {
        ...
        return _listBuckets().pluck('Buckets').flatMap(rxed).pluck('Name');
      };
      ...
      wrapper.deleteObject$ = function(url) {
        ...
        if (options.debug) console.log("Delete from s3", "bucket: " + bucket, "key: " + url);
        var obj = {
          Bucket: bucket,
          Key: url
        };
      ...
      wrapper.getObjectStream = function getObjectAsStream(url, range, pipe) {
        ...
        if (options.debug) console.log("Streaming from s3", "bucket: " + bucket, "key: " + url);
      ...
      wrapper.getObjectAsString = function getObjectAsStringMethod(url) {
        ...
        return rxo.defer(function() {
          ...
          .do(function(x) {
            ...
            if (options.debug) console.log("Fetched from s3", "bucket: " + bucket, "key: " + url + ' time: ' + finised_milliseconds);
          })
          .pluck('Body')
          .map(x => x.toString());
      };
      ...
      wrapper.putObjectAsString = function(url, acl) {
        return function(data) {
          ...
          return wrapper.putObject(obj)
            .do(function(x) {
              var finised_milliseconds = (new Date).getTime() - milliseconds;
              if (options.debug) console.log("Saved to s3", "bucket: " + bucket, "key: " + url + ' time: ' + finised_milliseconds);
            });
        }
    ...
    'Lambda': function(params) {
      ...
      wrapper.listFunctions = function() {
        return _listFunctions().pluck('Functions').flatMap(rxed);
      }
